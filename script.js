...
class Particle {
    ...
    draw() { ctx.beginPath(); ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2, false); ctx.fillStyle = '#64ffda'; ctx.fill(); }
    update() {
...
window.addEventListener('resize', () => { resizeCanvas(); mouse.radius = ((canvas.height/80) * (canvas.height/80)); initParticles(); });
initParticles(); 
animateBg();


// --- 4. THREE.JS 3D CORE (NEURAL NETWORK) ---
